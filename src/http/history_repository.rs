...
pub(super) async fn admin_internal_receive_history_repository_segment(
    ...
    // other repositories is best-effort and runs after the receipt response; a transient
    // callback failure must not pin the source outbox on the same tombstone forever.
    worker::schedule_tombstone_acknowledgement_fanout(
        &state,
        ready_repository_ids,
...
fn valid_repository_summary_cursor(cursor: &str) -> bool {
    let segment_id = cursor
        .strip_prefix("t:")
        .or_else(|| cursor.strip_prefix("r:"))
        .unwrap_or(cursor);
    segment_id.len() == 64 && segment_id.bytes().all(|byte| byte.is_ascii_hexdigit())
}
...
pub(super) async fn admin_internal_history_repository_summary(
    ...
    if query
        .after_segment_id
        .as_deref()
        .is_some_and(|cursor| !valid_repository_summary_cursor(cursor))
    {
        return Err(ApiError::invalid_request(
