...
use crate::{
    domain::{
        ...
        VlessCredentials, validate_cycle_day_of_month, validate_port, validate_tz_offset_minutes,
    },
    id::new_ulid_string,
    protocol::{
...
fn migrate_v5_to_v6(mut input: PersistedState) -> Result<PersistedState, StoreError> {
    ...
    // v6 hard-cut semantics:
    // - drop non-effective grants (`enabled=false`)
    // - drop orphan grants (missing user/endpoint)
    // - dedupe by (user_id, endpoint_id) with deterministic winner ordering
