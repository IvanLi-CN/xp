...
pub struct PersistedState {
    ...
    /// Durable, leader-resumable Raft membership work. This field is additive so a newer
    /// snapshot remains readable by an older binary until the capability barrier allows new
    /// command variants to be committed.
    #[serde(default)]
...
impl PersistedState {
    ...
    pub fn active_membership_operation(&self) -> Option<&MembershipOperation> {
        self.membership_operations
            .values()
            .find(|operation| operation.is_active())
    }
}
...
fn build_node_user_endpoint_memberships(
    ...
    let mut out = normalize_node_user_endpoint_memberships(
        state,
        state.node_user_endpoint_memberships.clone(),
    );
...
mod migrate_tests {
    ...
    fn migrate_v11_to_v12_infers_auto_assign_endpoint_kinds() {
        ...
        v11.endpoints
            .insert("ss_1".to_string(), migrate_test_ss_endpoint_one());
        v11.endpoints
            .insert("ss_2".to_string(), migrate_test_ss_endpoint_two());
        ...
        assert!(
            !v12.user_auto_assign_endpoint_kinds
...
pub enum DesiredStateCommand {
    ...
    BeginMembershipOperation {
        operation: Box<MembershipOperation>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        node: Option<Node>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        join_session: Option<JoinSession>,
    },
    TransitionMembershipOperation {
        operation: MembershipOperation,
    },
    PruneMembershipOperations {
...
impl DesiredStateCommand {
    pub fn apply(&self, state: &mut PersistedState) -> Result<DesiredStateApplyResult, StoreError> {
        if let Some(result) = membership_operation::apply_command(state, self) {
            return result;
        }
        match self {
            ...
            | Self::PruneMembershipOperations { .. } => {
                unreachable!("membership operation command was not handled")
            }
            ...
            Self::DeleteNode {
                ...
                if *delete_endpoints {
                    let actual_endpoint_ids = endpoint_refs
                        ...
                        .collect::<BTreeSet<_>>();
                    let expected_endpoint_ids = expected_endpoint_ids
                        .iter()
                        .cloned()
                        .collect::<BTreeSet<_>>();
            ...
            Self::UpsertEndpoint { endpoint, expected } => {
                ...
                if endpoint.kind == EndpointKind::VlessRealityVisionTcp {
                    ...
                    if meta.reality.server_names_source == RealityServerNamesSource::Global {
                        if !meta.managed_default {
                            meta.reality.dest =
                                format!("{}:443", meta.reality.server_names[0].trim());
                        }
                    } else {
                    ...
                    meta.accepted_authorities =
                        normalize_accepted_authorities(&meta.accepted_authorities).map_err(
                            |(reason, authority)| DomainError::InvalidAcceptedAuthority {
                                authority,
                                reason: reason.to_string(),
                            },
                        )?;
...
impl JsonSnapshotStore {
    ...
    pub fn latest_tcp_connection_usage_minute(&self) -> Option<chrono::DateTime<chrono::Utc>> {
    ...
    pub fn record_tcp_connection_usage_samples(
        ...
        let changed = self
            .tcp_connection_usage
            .record_minute_samples(minute, linux_only, warning, samples);
        if changed {
            self.save_tcp_connection_usage()?;
        }
        Ok(())
