...
"#
    ...
    let landing_filter = landing_group
        .get("filter")
        .and_then(Value::as_str)
        .expect("landing group should filter provider-hosted direct and chain proxies");
    ...
    let node_selector = proxy_groups
        .iter()
        .find(|group| group.get("name").and_then(Value::as_str) == Some("🚀 节点选择"))
        .expect("provider route should keep node selector");
    assert_eq!(
        node_selector
            .get("use")
            .and_then(Value::as_sequence)
