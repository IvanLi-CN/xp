...
async fn reconcile_once(config: &Config, handle: &DdnsHealthHandle) {
    ...
    let ipv4_probe = probe_public_ip(&config.cloudflare_ddns_ipv4_url, AddressFamily::Ipv4).await;
    let ipv6_probe = probe_public_ip(&config.cloudflare_ddns_ipv6_url, AddressFamily::Ipv6).await;
...
async fn apply_family_reconcile(
    ...
    match probe {
        ...
        ProbeOutcome::MissingCandidate(_reason) => {
            let family_state = state.family_mut(family);
        ...
        ProbeOutcome::Unknown(message) => Err(format!("ddns {} probe: {message}", family.label())),
    }
}
...
async fn probe_public_ip(url: &str, family: AddressFamily) -> ProbeOutcome {
    let parsed = match Url::parse(url) {
        Ok(url) => url,
        Err(err) => return ProbeOutcome::Unknown(format!("invalid url {url}: {err}")),
    };
    let Some(host) = parsed.host_str().map(ToOwned::to_owned) else {
        return ProbeOutcome::Unknown(format!("invalid url host for {url}"));
    };
    let port = parsed.port_or_known_default().unwrap_or(443);

    let resolved = if let Ok(ip) = host.parse::<IpAddr>() {
        if !family.matches_ip(ip) {
            return ProbeOutcome::MissingCandidate(format!(
        ...
        let addresses = match lookup {
            Ok(Ok(addrs)) => addrs,
            Ok(Err(err)) => return ProbeOutcome::Unknown(format!("dns lookup failed: {err}")),
            Err(err) => return ProbeOutcome::Unknown(format!("dns lookup task failed: {err}")),
        };
    ...
    let client = match builder.build() {
        Ok(client) => client,
        Err(err) => return ProbeOutcome::Unknown(format!("build probe client: {err}")),
    };
    ...
    if !response.status().is_success() {
        return ProbeOutcome::Unknown(format!("unexpected HTTP status {}", response.status()));
    }
    let body = match response.text().await {
        Ok(body) => body,
        Err(err) => return ProbeOutcome::Unknown(format!("read body failed: {err}")),
    };
    match parse_probe_ip(&body, family) {
        Ok(ip) => ProbeOutcome::Available(ip),
        Err(err) => ProbeOutcome::Unknown(err),
    }
}

fn parse_probe_ip(body: &str, family: AddressFamily) -> Result<IpAddr, String> {
...
fn classify_probe_error(err: reqwest::Error, family: AddressFamily) -> ProbeOutcome {
    if err.is_timeout() {
        return ProbeOutcome::Unknown("timeout".to_string());
    }
    ...
    ProbeOutcome::Unknown(err.to_string())
}
...
mod tests {
    ...
    fn trace_parser_accepts_plain_ip_and_trace_ip() {
        assert_eq!(
            parse_probe_ip("1.2.3.4\n", AddressFamily::Ipv4).unwrap(),
            "1.2.3.4".parse::<IpAddr>().unwrap()
        );
        assert_eq!(
            parse_probe_ip("fl=29f\nip=2001:db8::1\nts=1\n", AddressFamily::Ipv6).unwrap(),
