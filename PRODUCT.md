...
6. 使用项目组件承载可见控件。
   Web UI 不直接暴露浏览器原生控件外观或弹出层作为产品交互，例如
   `datalist`、原生 `select` 下拉、date/color picker。基础 HTML 元素可以作为组件内部的语义和可访问性基底。
   但可见外观、图标、弹层、状态色和键盘行为必须由本地 UI 组件体系承载。
   只有平台能力无法等价实现时才允许例外，并需要在对应实现或文档中写明理由、主题适配和可访问性边界。
