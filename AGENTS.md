...
- Host-managed initial joins provision Tunnel/DNS without starting `cloudflared`, then join the
  cluster and write `/etc/xp/xp.env` before activating `xray`, `xp`, and optional `cloudflared`
  in order with readiness checks. A joined node is successful only after its public
  `api_base_url/health` returns HTTP `200`; post-join health failure preserves membership and
  metadata for retry. Geo remains disabled by default and is written only by explicit
