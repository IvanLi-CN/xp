...
| 3e4q4 | Mihomo provider-only 动态链式订阅          | 已完成 | `3e4q4-mihomo-provider-dual-track/SPEC.md`              | 2026-06-19 | provider-only route；relay `url-test` 改为托管 VLESS 端口自身 `https://<access_host[:port]>/generate_204`；`🌟 {Region}` 冻结为 owner-facing 可见地区入口；managed-default VLESS 合同明确覆盖 host-managed systemd/OpenRC 与 Docker 节点                                                                                                          |
...
| 7qj3h | Web PWA 化与离线状态控制台                 | 已完成 | `7qj3h-web-pwa-offline-console/SPEC.md`                 | 2026-07-08 | fast-track；installable PWA shell + IndexedDB read cache + admin aggregated status SSE + offline read-only UX                                                                                                                                                                                                                                     |
