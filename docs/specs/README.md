...
| t8m4q | Xray 业务入站 stale TCP reclaim           | 待实现 | `t8m4q-xray-stale-tcp-reclaim/SPEC.md`                  | 2026-06-16 | fast-track；静态 `policy.levels.0` + 动态业务 inbound `socket_settings` + `xp-ops upgrade` rollout + shared testbox real-Xray gate                                                                                                                                                                                                               |
|       |                                            |        |                                                         |            |                                                                                                                                                                                                                                                                                                                                                   |
...
| rbt5e | 用户订阅 Mihomo 混入配置                   | 已完成 | `rbt5e-mihomo-subscription-template/SPEC.md`            | 2026-04-24 | PR #100 后续单链降载修订；`mixin_yaml` API、`🛣️ JP/HK/TW` 外层候选组、probe-derived 固定地区面、`💎 高质量` / `🚀 节点选择` 自动补点、系统托管引用清洗                                                                                                                                                                                             |
| 3e4q4 | Mihomo provider-only 动态链式订阅          | 已完成 | `3e4q4-mihomo-provider-dual-track/SPEC.md`              | 2026-06-16 | provider-only route；relay `url-test` 改为托管 VLESS 端口自身 `https://<access_host[:port]>/generate_204`；`🌟 {Region}` 冻结为 owner-facing 可见地区入口；managed-default VLESS 合同明确覆盖 host-managed systemd/OpenRC 与 Docker 节点                                                                                                          |
...
| c8qtw | 单 Docker 镜像集群节点部署                 | 已完成 | `c8qtw-docker-single-image-cluster-node-deploy/SPEC.md` | 2026-04-23 | fast-track；single-image Docker + GHCR multi-arch + optional Cloudflare Tunnel                                                                                                                                                                                                                                                                    |
