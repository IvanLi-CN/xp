...
- 若 repair 响应明确携带 `history_truncated=true`，且本地已经有该 source stream 的连续水位，
  ...
  若旧 marker 已存在但旧 response-complete 证据、active tiered handoff、watermark 推进、gap
  证据和完成范围均缺失，恢复 worker 必须重建该 handoff，并继续使用同一 response identity
  做幂等重试；旧 response-complete 证据仍表示该 stream allowance 已消费，不得再次跨越。
  普通 source delivery 和普通 anti-entropy 仍必须严格连续，不能用该标记伪造缺失 payload。
