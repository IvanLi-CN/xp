...
- Reality Mesh 和 Cloudflare Tunnel 是同级直连路径；选择稳定健康路径，另一条低频探测。
  两条都失败后才每小时抖动一次动态 Mesh relay。relay 不落盘，使用端到端 X25519+AEAD。
...
1. 管理员在 Raft 中指定仓库成员；新仓库进入 `syncing`，完成历史追赶且稳定五分钟后进入 `ready`。
2. source 将新 segment 按 cursor 提供给 primary；primary 写入本地 SQLite 并返回 ack；
   其他 ready 仓库通过 anti-entropy 修复缺口。
3. direct path 依据现有 Mesh/Tunnel 健康选择；只有两者均失败时才走 relay。
...
- source 重建数据库或序号丢失时生成新 epoch；cursor 过期返回 earliest available 和 gap。
