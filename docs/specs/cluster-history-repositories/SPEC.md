...
- Retention 对内容及聚合元信息均未变化的有界页面不得删除并重写历史行；游标持久化、过期删除及旧行元信息回填仍须执行。非法聚合元信息不得被视为未变化；查询在回填前须以有效的 observed
  区间报告不完整范围。原始行可合法地以 `complete=true` 且聚合范围为空持久化；可解析的聚合载荷若缺失有效范围，则必须以不完整状态持久化。
