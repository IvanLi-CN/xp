...
- ready repository 的初始回填可能只保留某条 source stream 的尾部 segment。若没有本地水位，
  非零首段必须携带已签名的 predecessor hash，初始回填专用接收路径才可以把它作为 retained
  anchor 持久化。若本地已有该 stream 的连续水位，且 repair 明确携带
  `history_truncated=true`，则首个 repair page 可以通过 tiered handoff 接受无 predecessor
  hash 的保留尾部首段，先记录被淘汰的前缀，再以该首段建立新的 hash-chain head。两种 retained
...
- 若重放 wire 的 SHA-256 segment ID 已经在接收仓库持久化，且其完整 cursor range 已落在当前
  ...
  这条路径不重新应用 payload，且在 runtime 重启后仍必须成立。相同序号但不同 segment ID
  仍按 fork 处理，不得借此放宽 fork 防护。
- 若 repair 响应明确携带 `history_truncated=true`，且本地已经有该 source stream 的连续水位，
  初始回填期间每个 source stream 可以允许一次服务端保留尾部首段作为新的 hash-chain head，
  跨过本地水位之后已被保留策略淘汰的序号；该 page 可能因 wire 上限拆为多个 response，
  也可能与其他 stream 分布在后续 summary page。接收方必须按 source、epoch、stream 持久化
  已消费的 stream allowance 和 `source_retention_expired` permanent gap；同一 stream 第二次
  handoff 仍拒绝。allowance marker 只表示历史消费记录，不代表 receiver watermark 或 permanent
  gap 已经落盘；完成的 handoff 还会持久化其范围，即使有界 gap ledger 后续轮转也不能再次跨越。
  若旧 marker 已存在但 active tiered handoff、watermark 推进、gap 证据和完成范围均缺失，
  恢复 worker 必须重建该 handoff，并继续使用同一 response identity 做幂等重试。旧
  response-complete 位是整个 repair response 的历史状态，不是按 stream 的完成证据；恢复
  不得用它阻塞另一个尚未完成的 stream。普通 source delivery 和普通 anti-entropy 仍必须
...
- Retention 对内容及聚合元信息均未变化的有界页面不得删除并重写历史行或重置分区摘要；
  游标持久化、过期删除及旧行元信息回填仍须执行，实际过期删除必须使摘要失效。
  非法聚合元信息不得被视为未变化；查询对显式非法的范围使用有效的 observed 区间报告不完整。
  原始行可合法地以 `complete=true` 且聚合范围为空持久化；可解析的聚合载荷若缺失有效范围，
  则必须以不完整状态持久化，并在再次压缩后保持不完整。
