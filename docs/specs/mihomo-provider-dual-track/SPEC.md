...
- `docs/specs/mihomo-provider-dual-track/contracts/http-apis.md`

## 计划资产（Plan assets）

- Directory: `docs/specs/mihomo-provider-dual-track/assets/`
...
- source_type=storybook_canvas · target_program=mock-only · capture_scope=element
  ...
  - submission_gate: `approved`
  - evidence_note: 托管 VLESS endpoint 详情页展示 `accepted_authorities`
...
- source_type=storybook_canvas · target_program=mock-only · capture_scope=element
  ...
  - evidence_note: 管理端 `New endpoint` 在 VLESS 创建路径只展示托管合同字段
