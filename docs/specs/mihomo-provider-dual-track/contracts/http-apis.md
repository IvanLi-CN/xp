...
- per-base relay 组只消费外部第三方 provider，不得使用 `DIRECT` 兜底，也不得 `use` `xp-system-generated`。
  有外部 provider 时通过日本/香港/新加坡 filter 做 `url-test` 主动探测，不得设置静态 `proxies` 或 `REJECT`
  候选，只设置 `empty-fallback: REJECT` 覆盖过滤为空或初始化状态；无外部 provider 时 relay 组只能使用 `REJECT` 拒绝哨兵，
