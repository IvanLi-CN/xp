...
- Monitor 使用 ULID；编辑创建递增不可变 revision，并在下一个 UTC Slot 生效。
...
- HTTP 默认 GET、可选 HEAD；默认接受 200-399，
  支持状态范围、3 次重定向和 literal contains。
- HTTPS 必须校验证书、主机名与有效期；TLS 失败不能降级。
...
- 执行器不使用环境代理、外部 ping 命令或 TCPING 作为 ICMP fallback。
...
- API 位于 `/api/admin/monitors`，沿用 admin token 与现有 Leader/follower 写语义。
...
- 两个 HTTP Observer 在同 Slot 都成功时，当前状态为 `up`，
  executed 与 success 都为 2。
- 同 Slot 一个成功、一个超时时状态为 `degraded`，错误为 `timeout`，
  两个分母各自正确。
- URL 或条件修改后，下一个 Slot 使用新 revision，旧 Observation 保留其 revision。
- journal 不可写时，不发送目标请求，并暴露 `capture_suspended` 或 quality gap。
- PING 无能力时标记 unsupported，不改用 TCPING。
- 私网初始解析或重定向地址在请求前被拒绝。
- Repository 不可用时历史返回 partial/local_only、watermark 与 gap，不伪装完整。
- 计划统计不包含 Ad Hoc Observation；过期状态显示 unknown 而非 down。
- 预算超过 300 Slot/分钟时，Leader 拒绝配置变更。
- Web 可完成 CRUD、暂停、立即检查和历史查看，并明确呈现 quality。
