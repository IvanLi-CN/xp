...
- 2026-06-16: 明确 `sockopt` 仅覆盖业务 inbound；`api` 保持控制面 loopback 语义，不纳入本次修复。
...
- 2026-06-17: 明确静态 config rewrite 必须保留自定义 `XP_XRAY_API_ADDR` listener，并清理退休的旧控制面代理 listener。
...
- 2026-06-18: 明确 shared testbox 子网选择还必须做远端串行 claim；仅凭“扫描当前空闲子网”不足以防止并发 compose run 抢占同一个 `10.203.x.0/24`。
- 2026-06-18: 明确 `xp-ops` 自升级后的恢复阶段若失败，旧 `xp-ops` 二进制也必须一并恢复，不能只回滚 `xp` 与 Xray static config。
