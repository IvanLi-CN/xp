...
- 新建 remote-config Tunnel 的首次配置 GET 若返回精确 Cloudflare `1055`，表示尚未存在远端
  ...
  必须保持终态失败并走补偿回滚。
- 当同名远端 Tunnel 与持久化的 XP account、zone、hostname、Tunnel ID 及本地凭据全部匹配时，
  非交互 deploy 必须直接复用该 Tunnel；不得将其当作名称冲突而生成随机 Tunnel 名称。
...
- 新建 Tunnel 首次配置读取返回 `1055` 时必须以空配置继续；既有 Tunnel 的相同响应不得被
  隐藏、重试或写入。
