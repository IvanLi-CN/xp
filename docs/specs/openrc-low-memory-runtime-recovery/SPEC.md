...
本规格承接既有 Xray 与 cloudflared 运行时自愈工作，并把小内存节点作为产品默认兼容目标。
...
- OpenRC 升级测试模拟 XP 仍运行时 runtime restart 被拒绝，
  并验证 `xp restart -> xp stop -> xray/cloudflared restart -> xp start` 的有序交接。
