...
- 跨节点 usage fan-out 应容忍短时边缘网络抖动，避免因瞬时高延迟过早把可达节点判成 unreachable。
...
- 对仍可在短时间窗口内返回的远端 usage 请求，应优先等待聚合结果，而不是把暂时高延迟直接判作失败。
...
- 2026-03-11: 补充跨节点 usage fan-out 的超时容忍要求，降低短时边缘网络抖动造成的误判 unreachable。
