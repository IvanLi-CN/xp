...
- Reverse Assignment 与 target-side Reverse Link 分离：assignment 仍是 durable topology，Link
  Lease、probe 和 circuit 仅为目标节点内存态。不可达 Rendezvous 在 10 秒 probe 后移除该 Link
  的 initiating outbound，并按 `30/60/120/240/300s` 重试；健康 replacement 才开始旧 Link 的
  120 秒 drain。`XP_REVERSE_MESH_ENABLED=false` 是不改 Raft 的本地 fail-closed 回退。
