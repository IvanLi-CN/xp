...
- 新加入且尚未应用认证 Raft state/snapshot 的非 bootstrap 节点必须保持本地 Mesh gate 关闭，
  ...
  Snapshot payload 持久化明确的 `mesh_state_applied` 证据与 snapshot identity；数据先于 metadata
  写入，读取时拒绝 identity 不匹配的文件对。旧 metadata 缺失该字段时，仅在 snapshot metadata
  与已应用日志一致且 payload marker 为 `true` 时恢复 gate，否则保持关闭。
