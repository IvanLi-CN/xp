...
- `🔒 高质量` 仍可通过 `use: [xp-system-generated]` 与 `filter` 动态包含 `{base}-reality` 接入点，并通过 `exclude-filter` 排除系统 `{base}-ss` 直连接入候选。
- transit-only 地区组 `🔒/🤯/🌟 {Japan|HongKong|Taiwan|Korea|Singapore|US|Other}` 不得 `use` `xp-system-generated`；它们只消费外部 `proxy-providers` 与命中地区 regex 的 `extra_proxies_yaml`。
- 地区组语义固定为：`🔒` = `select`，`🤯` = `url-test(interval=120,lazy=true)`，`🌟` = `fallback([🔒, 🤯])`。
...
- 当用户 profile 对应的 transit-only relay 候选为空时，payload 只保留 `{base}-ss` / `{base}-reality`，不输出 `*-chain`。
