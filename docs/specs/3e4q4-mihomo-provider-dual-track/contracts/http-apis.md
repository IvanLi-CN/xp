...
- `🛣️ JP/HK/SG` 只消费外部第三方 provider；无外部 provider 时回落 `DIRECT`，不得 `use` `xp-system-generated`。该外层隧道通过日本/香港/新加坡 filter 做 `url-test` 主动探测，不包含台湾候选。
...
- 所有 VLESS Reality 节点必须携带 Mihomo 运行时字段：`encryption: ""`、`packet-encoding: xudp`、`tls: true`、`client-fingerprint`、`skip-cert-verify: true`、`reality-opts` 与 `smux.enabled: false`。不得把 Reality/uTLS 指纹写入 Mihomo 证书 pinning `fingerprint` 字段。
