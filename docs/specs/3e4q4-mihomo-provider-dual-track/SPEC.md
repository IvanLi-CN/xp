...
- Given host-managed node upgrades from a legacy single-endpoint VLESS deployment, When the new `xp` version starts or `xp-ops xp sync-node-meta` runs, Then that lone VLESS endpoint is auto-adopted into the managed-default contract, including historical xp-created defaults that still persisted `managed_default=false`, but `reality.dest` is only rewritten after the loopback canary is ready; if canary preparation fails, the existing endpoint stays untouched and the blocker is exposed through `vless_https_canary_status.last_error`.
