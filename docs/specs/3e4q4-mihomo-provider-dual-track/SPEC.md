...
- Given host-managed node upgrades from a legacy single-endpoint VLESS deployment, When the new `xp` version starts or `xp-ops xp sync-node-meta` runs, Then that lone VLESS endpoint is auto-adopted into the managed-default contract and rewritten to the loopback canary semantics without requiring a manual endpoint patch.
