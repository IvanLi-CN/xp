...
- Given `/mihomo/provider/system` 返回 VLESS Reality 节点，Then 每个 VLESS Reality proxy 携带 Mihomo 运行时字段 `encryption`、`packet-encoding`、`tls`、`fingerprint`、`client-fingerprint`、`skip-cert-verify`、`reality-opts` 与 `smux`。
