...
- Given provider 方案同时存在 `base-reality` 与 `base-ss`，When 检查 hidden `🌟 {Region}` 与可见 `🔒 {Region}`，Then `🌟 {Region}` 作为底层地区组只承载该地区候选集合，`🔒 {Region}` 作为 owner-facing 入口只引用对应 `🌟 {Region}`，且两者都不直接暴露 `{base}-reality`、`{base}-ss-chain` 或 `{base}-reality-chain`。
...
- Given 新增节点完成主动探测并被归类到 `Taiwan`，When 请求 provider 主配置，Then hidden `🌟 Taiwan` 与可见 `🔒 Taiwan` 会自动建立对应地区入口关系，`🚀 节点选择` 会暴露 `🔒 Taiwan`，且 `🔒 高质量` 会稳定暴露该地区入口，无需更新用户模板。
