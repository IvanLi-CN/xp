...
- 节点配额编辑采用“锚定行头按钮的浮动层（popover）”，避免表格行内展开导致的 layout shift。
- 弹层定位以按钮为锚点（优先在下方；空间不足则在上方），并限制在视口内（clamp）。
- 弹层内容为：输入框在上，`Apply/Cancel` 在下（不再在右侧单独一列），整体保持紧凑宽度。
- 弹层按钮顺序为 `Cancel`（左）→ `Apply`（右），符合主动作在右侧的常见约定。
- 单位解析除 `MiB/GiB` 与 `MB/GB` 外，额外支持 `M/G` 简写。
...
补充需求（来自最新交互约束，见 #0017）：在“节点行”粒度，管理员还需要选择该节点的“流量重置配置”参考来源：参考用户配置或参考节点配置（默认参考用户配置）。Grant 本身不承载重置配置。
...
- 在节点行头提供“流量重置参考来源”选择：
  - 选项：参考用户配置 / 参考节点配置；
  - 默认：参考用户配置；
  - 该选择不进入 grant，而是进入“用户×节点”的配置（API 见本计划 contracts）。
- 配额输入支持单位并智能解析：
...
- 节点行头提供“流量重置参考来源”选择：
  - 选项：参考用户配置 / 参考节点配置；
  - 默认：参考用户配置；
  - 写入通过本计划的 `UserNodeQuota` API（字段 `quota_reset_source`）。
- 单位解析规则明确且可单元测试：
  - 支持 `GiB/MiB` 及常见变体；
  - 无单位默认 `MiB`；
  - 允许小数并四舍五入到 bytes；
...
- Given 用户详情页的 Access & quota 表格已加载，
  When 用户将某节点行头的“参考来源”从“参考用户配置”切换为“参考节点配置”并提交保存，
  Then UI 发送 `PUT /api/admin/users/:user_id/node-quotas/:node_id`（含 `quota_reset_source: "node"`），
  And 保存成功后该节点行头展示为“参考节点配置”。
