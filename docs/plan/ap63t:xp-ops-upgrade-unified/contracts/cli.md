...
The following legacy commands remain runnable but are hidden from `--help` output and must print a
deprecation warning to stderr:

- `xp-ops self-upgrade` → prefer `xp-ops upgrade`
- `xp-ops xp upgrade` → prefer `xp-ops upgrade`
