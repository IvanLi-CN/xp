...
- [x] M1: 重做 Subscription 区块布局（增大预览区域 + 一键展开）
- [x] M2: 引入/实现代码式预览组件（JSON/YAML/text：不折行 + 横向滚动 + 等宽；语法高亮 + 行号冻结）
- [x] M3: 增加关键字段复制快捷入口（至少 `public-key` / `short-id` / `servername`）
- [x] M4: 补齐 E2E 覆盖（raw + clash + clipboard 断言）
...
- 2026-01-22: 实现交付：入口页移除 inline preview；Fetch=打开弹窗并拉取；弹窗代码视图不折行+横向滚动+行号+语法高亮+搜索；右栏关键字段一键复制；补齐 e2e 覆盖
- 2026-01-22: 修正：右侧 Fields 面板按草图完善（中间截断展示 + Copy all fields + 提示文案）
- 2026-01-22: 细节：弹窗右上角关闭按钮改为图标，并同步更新设计草图
- 2026-01-22: 细节：关闭图标按钮放大点击区，避免过小
- 2026-01-22: 细节：关闭按钮本体采用圆形（更易点击且与动作区一致）
- 2026-01-22: 修正：代码视图改为固定高度深色面板（对齐设计图信息密度）
- 2026-01-22: 调整：代码视图改为自适应高度（clash 时最小对齐 Fields，否则按内容高度）
- 2026-01-22: 修正：clash 时代码面板填充到右侧 Fields 高度（消除面板下方空白区域）
- 2026-01-22: 细节：代码面板增加上下内边距（对齐设计图留白）
- 2026-01-22: 细节：clash 模式 Code/Fields 固定为同高 `56vh`（对齐草图），并微调代码面板配色/圆角与关闭按钮尺寸，同时同步更新设计 SVG
- 2026-01-22: 修正：弹窗顶部区域对齐设计（Title + format pill + “no wrap · scroll” + Search 行归入 header，并调整顶部按钮样式）
- 2026-01-22: 修正：`Find next/prev` 对齐到 `Copy` 按钮组（Close 独立一列；两行按钮右侧对齐规则一致）
- 2026-01-22: 细节：顶部 `Copy/Find/Search` 控件按设计图定制配色与尺寸（基于 light/dark 主题切换），避免 Daisy 默认样式导致偏差
- 2026-01-22: 细节：Storybook 的 Clash 样例内容与设计图对齐；代码区使用固定伪滚动条与单色代码配色；弹窗遮罩颜色与设计图一致
- 2026-01-23: 修正：Storybook iframe 默认 body padding 导致弹窗溢出裁切；清零 padding 并消除横向滚动
- 2026-01-23: 修正：窄视口下 Code/Fields 栅格溢出导致右侧裁切；改为可收缩列布局以保持可用
- 2026-01-23: 修正：窄视口下 Code/Fields 纵向堆叠时 Fields 被“下方挤出视口”；限制代码区高度并让 Fields 在首屏可见（仍可滚动查看更多）
- 2026-01-23: 细节：窄视口下 Fields 值输入框改为自适应占满剩余宽度（Copy 按钮固定宽），避免右侧空间浪费
- 2026-01-23: 修正：中等宽度下 Header 改为双列布局（Search 与 Find next/prev 同行），减少顶部区域空洞与额外行高
- 2026-01-23: 细节：Search 输入与 Find next/prev 之间增加列间距，避免控件“粘连”影响可读性
- 2026-01-23: 细节：移除头部 “no wrap · scroll” 标签（按反馈简化信息层级）
- 2026-01-23: 修正：关闭按钮改为固定在弹窗右上角（不同宽度/可滚动时仍保持可见）
- 2026-01-23: 修正：代码区自定义滚动条改为真实滚动条（thumb 随滚动与内容尺寸变化，并支持拖拽）
- 2026-01-23: 修正：Fields 面板不再内部滚动；容器高度随内容增长，需要滚动时由弹窗整体滚动承担
- 2026-01-23: 修正：Search 行改为占满整行宽度（输入框与按钮组不重叠且有间隔）；Find next/prev 按钮宽度自适应内容并右对齐
- 2026-01-23: 细节：Header 区域恢复右侧内边距（避免右侧控件贴边）
- 2026-01-23: 细节：Search 行右侧为 Close 预留安全区，使 Find 按钮右边与内容区域对齐（不再凸出）
- 2026-01-23: 增强：代码视图补齐语法高亮（YAML/JSON）并为 Search 匹配提供可见高亮（active line + match 背景）
- 2026-01-23: 细节：统一弹窗内容右侧 padding（Copy/Find 与设计草图右边距一致）；窄宽度下 Search 行允许换行，输入框优先占满宽度且按钮组保持右对齐
- 2026-01-23: 修正：头部按钮避免被右上角 Close 覆盖（为 Close 单独预留右侧 lane，并让 Copy 与 Find 的右边界一致）
- 2026-01-23: 设计修正：对齐线以 Close 右边界为基准（Find prev 右边对齐 Close），并同步更新 redesign SVG（dark/light）
