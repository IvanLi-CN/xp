...
- Each VLESS endpoint may store its own `canary_upstream`. When it is unset, non-probe requests return a diagnostic error. When set, xp forwards method, path, query, headers, status, response headers, and streaming bodies to that endpoint upstream. Upstream mode is `auto`, `http1`, or explicit `h2c`; `auto` supports HTTP/1.1 and HTTPS ALPN HTTP/2.
- The reverse proxy is TLS-terminating HTTP reverse proxy behavior, not TCP passthrough and not a forward proxy. It supports streaming request/response bodies, SSE, large uploads/downloads, and WebSocket upgrade; `CONNECT` is not part of the v1 contract.
...
- The endpoint detail page exposes a managed VLESS **Canary /generate_204** test for that ordinary HTTPS path. It is an immediate diagnostic for public ingress, TLS, REALITY fallback, and xp canary behavior; it is separate from the hourly cluster-wide proxy path probe and is not stored in endpoint probe history.
