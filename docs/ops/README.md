...
- Each VLESS endpoint may store its own `canary_upstream` origin URL. When it is unset, non-probe requests return a diagnostic error. When set, xp forwards method, path, query, headers, status, response headers, and streaming bodies to that endpoint upstream. Upstream mode is `auto`, `http1`, or explicit `h2c`; `auto` supports HTTP/1.1 and HTTPS ALPN HTTP/2.
