...
protocol errors, and any response carrying a signed acknowledgement are never retried. A
connection/DNS/TLS failure or timeout before response headers uses the same bounded retry only for
those idempotent request classes; after the retry budget is exhausted, the operation remains
outcome-unknown and a History request does not advance its ACK.
...
私网访问采用节点本地 CIDR 策略。部署默认只来自 `XP_MIHOMO_ALLOWED_PRIVATE_CIDRS`，例如 `<operator-private-cidrs>`；环境变量、节点详情页 API 和覆盖文件均接受裸私网 IP，服务端统一规范为主机 CIDR（IPv4 `/32`、原生 ULA IPv6 `/128`）。节点详情页可通过 `GET|PUT /api/admin/nodes/{node_id}/mihomo-resource-policy` 覆盖，DELETE 覆盖路径可恢复部署默认。覆盖保存于 `${XP_DATA_DIR}/mihomo-resource-policy.json`，不进入 Raft。未获准的私网、loopback、链路本地、保留和文档地址返回 `403 private_target_blocked`；公网地址仍可直接访问。旧 `/api/admin/mihomo/resource-policy` GET 固定返回 `false`，PUT 在兼容窗口内返回 `410 Gone`。
...
- Repository bootstrap is bounded and resumable: one worker tick handles at most one local page
  ...
  The persisted retained-anchor stream marker is normally historical allowance evidence, not
  completion evidence. For legacy checkpoints, however, the matching
  `retained_anchor_repair_response_seen` bit is completion evidence and blocks a second handoff
  for that stream. If an older release left only the marker while that bit, the tiered handoff,
  receiver watermark advance, and permanent gap are absent, the next public retry recreates the
  handoff and reuses the response identity; do not edit the checkpoint or clear the backlog
  manually.
  ...
  retry. If index creation fails, XP preserves the external-history SQLite rows and marks its
  history storage unavailable rather than falling back to JSON; repair the local storage condition
  before retrying.
