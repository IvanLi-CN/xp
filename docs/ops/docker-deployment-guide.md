...
- 每个托管 VLESS endpoint 可单独配置 origin-only `canary_upstream`，也可配置 `accepted_authorities` 别名集合；`GET/HEAD /generate_204` 永远由 xp 返回 `204`，其他请求按 HTTP `Host`/`:authority` 匹配 canonical `XP_ACCESS_HOST[:endpoint_port]` 或别名集合后转发到该 endpoint upstream；转发时不透传 hop-by-hop headers，发往 upstream 的 `Host` 按 `canary_upstream` origin 归一化；未命中或未配置 upstream 时对外统一返回普通纯文本 `404 Not Found`
