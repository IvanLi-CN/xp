...
- 每个 VLESS endpoint 可单独配置 origin-only `canary_upstream`；`GET/HEAD /generate_204` 永远由 xp 返回 `204`，其他请求按 HTTP `Host`/`:authority` 匹配 `XP_ACCESS_HOST[:endpoint_port]` 后转发到该 endpoint upstream；转发时不透传 hop-by-hop headers，发往 upstream 的 `Host` 按 `canary_upstream` origin 归一化
