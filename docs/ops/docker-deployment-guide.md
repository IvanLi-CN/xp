...
这份文档只描述官方单镜像容器运行时，也就是“官方单镜像容器节点”这一类环境。

如果你的节点是：

- 直接在宿主机上运行 `xp + xray + cloudflared`
- 使用 systemd 或 OpenRC 管理服务
- 历史上不是通过 `xp-ops container run` 接管

那它属于 host-managed 部署，应改看 `docs/ops/README.md`，不要把本页流程硬套到宿主机托管节点上。
...
- `GET /api/health` 里的 `vless_https_canary` 有合理运行态
- 对托管 VLESS 端口执行 `curl -Ik https://<access_host[:vless_port]>/generate_204` 返回 `204`

## 托管 VLESS HTTPS canary 语义
...
- `XP_DEFAULT_VLESS_SERVER_NAMES` 仅作为旧 bootstrap 配置兼容输入存在；没有
  `XP_DEFAULT_VLESS_PORT` 时会被忽略，bootstrap 生效时会校验但不决定托管 SNI
- 每个托管 VLESS endpoint 可单独配置 origin-only `canary_upstream`，也可配置 `accepted_authorities` 别名集合；别名格式为 `host[:port]`，省略端口时按 HTTPS 默认 `443` 解释。`GET/HEAD /generate_204` 永远由 xp 返回 `204`，其他请求按 HTTP `Host`/`:authority` 匹配 canonical `XP_ACCESS_HOST[:endpoint_port]` 或别名集合后转发到该 endpoint upstream；转发时不透传 hop-by-hop headers，发往 upstream 的 `Host` 按 `canary_upstream` origin 归一化；未命中或未配置 upstream 时对外统一返回普通纯文本 `404 Not Found`
