...
- per-base relay 组只消费外部第三方 provider，避免系统 `*-chain` 递归指回自身；有外部 provider 时通过日本/香港/新加坡 filter 做 `url-test`，不得保留 `DIRECT` 兜底；无外部 provider 时 relay 组只能使用 `REJECT` 拒绝哨兵，provider 候选被 filter 筛空时也不得回落直连。健康检查 URL 选择顺序为：
