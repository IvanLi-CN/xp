...
- 样式：Tailwind CSS v4 + shadcn/ui（Radix primitives + 本地组件库）
  - 主题：`xp-light` / `xp-dark`（通过 `UiPrefs` 同步 `<html data-theme>` 与 `dark` class 生效）
  - 偏好配置：`xp_ui_theme`（system/light/dark）与 `xp_ui_density`（comfortable/compact）存储在 `localStorage`
  - 表单：统一使用 `react-hook-form + zod + @hookform/resolvers` 组织校验与提交
  - 通知：统一使用 Sonner；危险操作统一走 AlertDialog
